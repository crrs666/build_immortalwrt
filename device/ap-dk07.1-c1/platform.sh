...
platform_check_image() {
	case "$(board_name)" in
	qcom,ap-dk07.1-c1)
		# 厂商 ubi0(mtd13 "rootfs") 上是 kernel(36 LEB) + ubi_rootfs(681 LEB)，
		# 728 个好 PEB 几乎被占满。OpenWrt 升级只删 kernel/rootfs/rootfs_data
		# 三个同名卷，不会动 ubi_rootfs，导致新卷无空间可建；且此时旧 kernel
		# 卷已被删除，重启即变砖。故先要求手工删除厂商卷。
		CI_UBIPART="rootfs"
		...
		cat << EOF
Stock ubi_rootfs volume is still present.
There's not enough space left
to install the OpenWrt kernel/rootfs volumes.

You need to delete the stock volume first:
# ubirmvol /dev/ubi0 --name=ubi_rootfs
Please also delete ubi_rootfs_data, if exist:
# ubirmvol /dev/ubi0 --name=ubi_rootfs_data
...
platform_do_upgrade() {
	...
	wallys,dr40x9)
		...
		;;
	qcom,ap-dk07.1-c1)
		# 本板 UBI 容器分区名是 "rootfs"(mtd13)，nand.sh 默认 CI_UBIPART="ubi"，
		# 不覆盖则 nand_attach_ubi 找不到分区（dock 上另有一个 "data" UBI 分区，
		# 本系统不使用）。kernel/rootfs 卷名与默认值一致，无需另设。
		CI_UBIPART="rootfs"
		nand_do_upgrade "$1"
		;;
